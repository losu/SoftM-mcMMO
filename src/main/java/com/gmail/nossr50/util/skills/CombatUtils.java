...
public final class CombatUtils {
	...
	private static boolean harmfulCreature;

	public final static double LIFE_STEAL_PCTG = 0.6;

	private static void processSwordCombat(LivingEntity target, Player player,
	...
	private static void processAxeCombat(LivingEntity target, Player player,
			EntityDamageByEntityEvent event) {
		double initialDamage = event.getDamage();
	...
	private static void processUnarmedCombat(LivingEntity target,
			Player player, EntityDamageByEntityEvent event) {
		double initialDamage = event.getDamage();
		double finalDamage = initialDamage;
	...
	private static void processTamingCombat(LivingEntity target, Player master,
			Wolf wolf, EntityDamageByEntityEvent event) {
		double initialDamage = event.getDamage();
		double finalDamage = initialDamage;
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		if (commandLabel.equalsIgnoreCase("wolf")) {
			...
			Wolf wolf = (Wolf) player.getWorld().spawnEntity(location,
					EntityType.WOLF);
			wolf.setAngry(true);
	...
	private static void processArcheryCombat(LivingEntity target,
			Player player, EntityDamageByEntityEvent event, Arrow arrow) {
		double initialDamage = event.getDamage();
		double finalDamage = initialDamage;
		...
		if (target instanceof Player && SkillType.UNARMED.getPVPEnabled()) {
			UnarmedManager unarmedManager = UserManager.getPlayer(
					(Player) target).getUnarmedManager();
		...
		if (!arrow.hasMetadata(mcMMO.infiniteArrowKey)
		...
		startGainXp(mcMMOPlayer, target, SkillType.ARCHERY,
				arrow.getMetadata(mcMMO.bowForceKey).get(0).asDouble());
	}
	 ...
	 */
	public static void processCombatAttack(EntityDamageByEntityEvent event,
			Entity attacker, LivingEntity target) {
		...
		if (attacker instanceof Player && entityType == EntityType.PLAYER) {
			...
			if (target instanceof Tameable) {
				if (heldItem.getType() == Material.BONE) {
					TamingManager tamingManager = UserManager.getPlayer(player)
							.getTamingManager();
			...
			/*
			 * Implementation of lifesteal
			 * 
			 * PRE: the player is the attacker, the target is a harmful creature
			 * and the player's health is not full
			 * 
			 * POST: a percentage of the damage inflicted might be returned to
			 * the player
			 */
			harmfulCreature = isHarmfulEntity(target);
			double currentHealth = player.getHealth();
			double maxHealth = player.getMaxHealth();
			if (harmfulCreature && currentHealth != maxHealth) {
				double dam = event.getDamage();
				double pctg = Math.random();

				// 60% chance of not getting health points back
				if (pctg >= LIFE_STEAL_PCTG) {
					pctg = Math.random();
					Double addHealth = dam * pctg;
					double newHealth = currentHealth + addHealth;
					...
					String message = "You have been given "
							+ addHealth.toString() + " HP";
			...
			if (tamer != null && tamer instanceof Player
					&& SkillType.TAMING.shouldProcess(target)) {
				Player master = (Player) tamer;

				if (!Misc.isNPCEntity(master)
						&& SkillType.TAMING.getPermissions(master)) {
			...
			if (projectileSource != null && projectileSource instanceof Player
					&& SkillType.ARCHERY.shouldProcess(target)) {
				Player player = (Player) projectileSource;

				if (!Misc.isNPCEntity(player)
						&& SkillType.ARCHERY.getPermissions(player)) {
		...
		if (target instanceof Player) {
			...
			McMMOPlayer mcMMOPlayer = UserManager.getPlayer(player);
			AcrobaticsManager acrobaticsManager = mcMMOPlayer
					.getAcrobaticsManager();
			...
			if (ItemUtils.isSword(player.getItemInHand())) {
				...
				if (swordsManager.canUseCounterAttack(damager)) {
					swordsManager.counterAttackChecks((LivingEntity) damager,
							event.getDamage());
				}
			}
		}
	 ...
	 */
	public static void dealDamage(LivingEntity target, double damage,
			Map<DamageModifier, Double> modifiers, LivingEntity attacker) {
		if (target.isDead()) {
			return;
		}
	...
	@Deprecated
	public static void dealDamage(LivingEntity target, double damage,
			DamageCause cause, Entity attacker) {
		if (target.isDead()) {
			return;
		}
	 ...
	 */
	public static void applyAbilityAoE(Player attacker, LivingEntity target,
			double damage, Map<DamageModifier, Double> modifiers, SkillType type) {
		int numberOfTargets = getTier(attacker.getItemInHand()); // The higher
																	// the
																	// weapon
																	// tier, the
																	// more
		...
		for (Entity entity : target.getNearbyEntities(2.5, 2.5, 2.5)) {
			...
			if (Misc.isNPCEntity(entity) || !(entity instanceof LivingEntity)
			...
			case SWORDS:
				if (entity instanceof Player) {
					((Player) entity).sendMessage(LocaleLoader
							.getString("Swords.Combat.SS.Struck"));
				}

				BleedTimerTask.add(livingEntity,
						Swords.serratedStrikesBleedTicks);
				break;

			case AXES:
				if (entity instanceof Player) {
					((Player) entity).sendMessage(LocaleLoader
							.getString("Axes.Combat.SS.Struck"));
				}
	...
	public static void startGainXp(McMMOPlayer mcMMOPlayer,
			LivingEntity target, SkillType skillType) {
	 ...
	 */
	private static void startGainXp(McMMOPlayer mcMMOPlayer,
			LivingEntity target, SkillType skillType, double multiplier) {
		double baseXP = 0;
		XPGainReason xpGainReason;

		if (target instanceof Player) {
			if (!ExperienceConfig.getInstance()
					.getExperienceGainsPlayerVersusPlayerEnabled()) {
				return;
			}
			...
			if (defender.isOnline()
					&& SkillUtils.cooldownExpired(mcMMOPlayer.getRespawnATS(),
							Misc.PLAYER_RESPAWN_COOLDOWN_SECONDS)) {
				baseXP = 20 * ExperienceConfig.getInstance()
						.getPlayerVersusPlayerXP();
			}
		} else {
			if (mcMMO.getModManager().isCustomEntity(target)) {
				baseXP = mcMMO.getModManager().getEntity(target)
						.getXpMultiplier();
				...
				case SKELETON:
					...
					case WITHER:
						baseXP = ExperienceConfig.getInstance()
								.getWitherSkeletonXP();
						break;
					default:
						baseXP = ExperienceConfig.getInstance().getCombatXP(
								type);
						break;
					}
					break;
		...
		if (baseXP != 0) {
			new AwardCombatXpTask(mcMMOPlayer, skillType, baseXP, target,
					xpGainReason).runTaskLater(mcMMO.p, 0);
		}
	}

	/**
	 * Check to see if the given LivingEntity should be affected by a combat
	 * ability.
	 *
	 * @param player
	 *            The attacking Player
	...
	private static boolean shouldBeAffected(Player player, Entity entity) {
		if (entity instanceof Player) {
			...
			if (!defender.getWorld().getPVP() || defender == player
					|| UserManager.getPlayer(defender).getGodMode()) {
				return false;
			}

			if ((PartyManager.inSameParty(player, defender) || PartyManager
					.areAllies(player, defender))
					&& !(Permissions.friendlyFire(player) && Permissions
							.friendlyFire(defender))) {
				return false;
			}
			...
			// It may seem a bit redundant but we need a check here to prevent
			// bleed from being applied in applyAbilityAoE()
			if (callFakeDamageEvent(player, entity, 1.0) == 0) {
				return false;
			}
		} else if (entity instanceof Tameable) {
			if (isFriendlyPet(player, (Tameable) entity)) {
				// isFriendlyPet ensures that the Tameable is: Tamed, owned by a
				// player, and the owner is in the same party
				// So we can make some assumptions here, about our casting and
				// our check
				Player owner = (Player) ((Tameable) entity).getOwner();
				if (!(Permissions.friendlyFire(player) && Permissions
						.friendlyFire(owner))) {
					return false;
				}
			}
	...
	public static boolean isInvincible(LivingEntity entity, double eventDamage) {
		 ...
		 */
		return (entity.getNoDamageTicks() > entity.getMaximumNoDamageTicks() / 2.0F)
				&& (eventDamage <= entity.getLastDamage());
	}
		...
		if (pet.isTamed()) {
			...
			if (tamer instanceof Player) {
				...
				return (owner == attacker
						|| PartyManager.inSameParty(attacker, owner) || PartyManager
							.areAllies(attacker, owner));
			}
		}
	...
	public static double callFakeDamageEvent(Entity attacker, Entity target,
			double damage) {
		return callFakeDamageEvent(
				attacker,
				target,
				DamageCause.ENTITY_ATTACK,
				new EnumMap<DamageModifier, Double>(ImmutableMap.of(
						DamageModifier.BASE, damage)));
	}

	@Deprecated
	public static double callFakeDamageEvent(Entity attacker, Entity target,
			DamageCause damageCause, double damage) {
		EntityDamageEvent damageEvent = attacker == null ? new FakeEntityDamageEvent(
				target, damageCause, damage)
				: new FakeEntityDamageByEntityEvent(attacker, target,
						damageCause, damage);
	...
	public static double callFakeDamageEvent(Entity attacker, Entity target,
		...
		return callFakeDamageEvent(attacker, target, DamageCause.ENTITY_ATTACK,
				modifiers);
	}

	public static double callFakeDamageEvent(Entity attacker, Entity target,
			double damage, Map<DamageModifier, Double> modifiers) {
		return callFakeDamageEvent(attacker, target, DamageCause.ENTITY_ATTACK,
				getScaledModifiers(damage, modifiers));
	}

	public static double callFakeDamageEvent(Entity attacker, Entity target,
			DamageCause cause, Map<DamageModifier, Double> modifiers) {
		EntityDamageEvent damageEvent = attacker == null ? new FakeEntityDamageEvent(
				target, cause, modifiers) : new FakeEntityDamageByEntityEvent(
				attacker, target, cause, modifiers);
		mcMMO.p.getServer().getPluginManager().callEvent(damageEvent);
	...
	private static Map<DamageModifier, Double> getModifiers(
	...
	private static Map<DamageModifier, Double> getScaledModifiers(
			double damage, Map<DamageModifier, Double> modifiers) {
	...
	public static EntityDamageByEntityEvent applyScaledModifiers(
			double initialDamage, double finalDamage,
			EntityDamageByEntityEvent event) {
		...
		for (DamageModifier modifier : DamageModifier.values()) {
			...
			event.setDamage(modifier,
					finalDamage / initialDamage * event.getDamage(modifier));
		}
	...
	public static void handleHealthbars(Entity attacker, LivingEntity target,
			double damage) {
	 ...
	 * 
	 * POST: returns true if target can inflict damage to the player and false
	 * otherwise
	 */
	public static boolean isHarmfulEntity(LivingEntity target) {
		EntityType type = target.getType();
