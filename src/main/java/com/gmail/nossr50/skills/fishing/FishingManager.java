...
public class FishingManager extends SkillManager {
	...
	public boolean canShake(Entity target) {
		return target instanceof LivingEntity
				&& getSkillLevel() >= Tier.ONE.getLevel()
				&& Permissions.secondaryAbilityEnabled(getPlayer(),
						SecondaryAbility.SHAKE);
	}

	public boolean canMasterAngler() {
		return getSkillLevel() >= AdvancedConfig.getInstance()
				.getMasterAnglerUnlockLevel()
				&& Permissions.secondaryAbilityEnabled(getPlayer(),
						SecondaryAbility.MASTER_ANGLER);
	}
	...
	private boolean unleashTheKraken(boolean forceSpawn) {
		if (!forceSpawn
				&& (fishingTries < AdvancedConfig.getInstance()
						.getKrakenTriesBeforeRelease() || fishingTries <= Misc
						.getRandom().nextInt(200))) {
			return false;
		}
		...
		String unleashMessage = AdvancedConfig.getInstance()
				.getPlayerUnleashMessage();
		...
		if (globalEffectsEnabled) {
			...
			world.playSound(location, Sound.GHAST_SCREAM, Misc.GHAST_VOLUME,
					Misc.getGhastPitch());
			...
			world.createExplosion(location.getX(), location.getY(),
					location.getZ(), 0F, false, false);
			world.createExplosion(location.getX(), location.getY(),
					location.getZ(), 0F, false, false);
			world.createExplosion(location.getX(), location.getY(),
					location.getZ(), 0F, false, false);

			player.playSound(location, Sound.GHAST_SCREAM, Misc.GHAST_VOLUME,
					Misc.getGhastPitch());
		}
		...
		Creature kraken = (Creature) world.spawnEntity(player.getEyeLocation(),
				(Misc.getRandom().nextInt(100) == 0 ? EntityType.CHICKEN
						: EntityType.SQUID));
		kraken.setCustomName(AdvancedConfig.getInstance().getKrakenName());

		if (!kraken.isValid()) {
			int attackInterval = AdvancedConfig.getInstance()
					.getKrakenAttackInterval() * Misc.TICK_CONVERSION_FACTOR;
			new KrakenAttackTask(kraken, player, player.getLocation())
					.runTaskTimer(mcMMO.p, attackInterval, attackInterval);
		...
		int attackInterval = AdvancedConfig.getInstance()
				.getKrakenAttackInterval() * Misc.TICK_CONVERSION_FACTOR;
		new KrakenAttackTask(kraken, player).runTaskTimer(mcMMO.p,
	...
	public boolean exploitPrevention() {
		...
		Block targetBlock = getPlayer().getTargetBlock(
				BlockUtils.getTransparentBlocks(), 100);
		...
		boolean hasFished = (currentTime < fishingTimestamp
				+ FISHING_COOLDOWN_SECONDS);

		fishingTries = hasFished ? fishingTries + 1 : Math.max(
				fishingTries - 1, 0);
		fishingTimestamp = currentTime;

		Location targetLocation = targetBlock.getLocation();
		boolean sameTarget = (fishingTarget != null && fishingTarget
				.equals(targetLocation));

		fishingTries = sameTarget ? fishingTries + 1 : Math.max(
				fishingTries - 1, 0);
	...
	public boolean canIceFish(Block block) {
		if (getSkillLevel() < AdvancedConfig.getInstance()
				.getIceFishingUnlockLevel()) {
			return false;
		}
		...
		if (!Fishing.iceFishingBiomes.contains(block.getBiome())
				&& (block.getRelative(BlockFace.DOWN, 3).getType() != Material.STATIONARY_WATER)) {
			return false;
		}

		Player player = getPlayer();

		if (!Permissions.secondaryAbilityEnabled(getPlayer(),
				SecondaryAbility.ICE_FISHING)) {
			return false;
		}
	 ...
	 *
	 * @param rankChange
	 *            The # of levels to change rank for the food
	 * @param eventFoodLevel
	 *            The initial change in hunger from the event
	 *
	 * @return the modified change in hunger for the event
	 */
	public int handleFishermanDiet(int rankChange, int eventFoodLevel) {
		return SkillUtils.handleFoodSkills(getPlayer(), skill, eventFoodLevel,
				Fishing.fishermansDietRankLevel1,
				Fishing.fishermansDietMaxLevel, rankChange);
	}
	 ...
	 *
	 * @param fishingCatch
	 *            The {@link Item} initially caught
	 */
	public void handleFishing(Item fishingCatch) {
		...
		if (treasure != null) {
			ItemStack treasureDrop = treasure.getDrop().clone(); // Not cloning
																	// is bad,
																	// m'kay?
			Map<Enchantment, Integer> enchants = new HashMap<Enchantment, Integer>();

			if (Permissions.secondaryAbilityEnabled(player,
					SecondaryAbility.MAGIC_HUNTER)
					&& ItemUtils.isEnchantable(treasureDrop)) {
				enchants = handleMagicHunter(treasureDrop);
			}

			McMMOPlayerFishingTreasureEvent event = EventUtils
					.callFishingTreasureEvent(player, treasureDrop,
			...
			// Drop the original catch at the feet of the player and set the
			// treasure as the real catch
			if (treasureDrop != null) {
				...
				if (enchanted) {
					player.sendMessage(LocaleLoader
							.getString("Fishing.Ability.TH.MagicFound"));
				}

				if (Config.getInstance().getFishingExtraFish()) {
					Misc.dropItem(player.getEyeLocation(),
							fishingCatch.getItemStack());
				}
	 ...
	 *
	 * @param experience
	 *            The amount of experience initially awarded by the event
	 *
	 ...
	 *
	 * @param target
	 *            The {@link LivingEntity} affected by the ability
	 */
	public void shakeCheck(LivingEntity target) {
		...
		SecondaryAbilityWeightedActivationCheckEvent activationEvent = new SecondaryAbilityWeightedActivationCheckEvent(
				getPlayer(), SecondaryAbility.SHAKE, getShakeProbability()
						/ activationChance);
		mcMMO.p.getServer().getPluginManager().callEvent(activationEvent);
		if ((activationEvent.getChance() * activationChance) > Misc.getRandom()
				.nextInt(activationChance)) {
			List<ShakeTreasure> possibleDrops = Fishing
					.findPossibleDrops(target);
			...
			// It's possible that chooseDrop returns null if the sum of
			// probability in possibleDrops is inferior than 100
			if (drop == null) {
				return;
			}
			...
			case SHEEP:
				...
				if (drop.getType() == Material.WOOL) {
					...
					drop = new Wool(sheep.getColor()).toItemStack(drop
							.getAmount());
			...
			McMMOPlayerShakeEvent shakeEvent = new McMMOPlayerShakeEvent(
					getPlayer(), drop);
			...
			CombatUtils.dealDamage(target,
					Math.max(target.getMaxHealth() / 4, 1)); // Make it so you
																// can shake a
																// mob no more
																// than 4 times.
	 ...
	 *
	 * @return The {@link FishingTreasure} found, or null if no treasure was
	 *         found.
	 */
	...
	private FishingTreasure getFishingTreasure() {
		...
		diceRoll -= getPlayer().getItemInHand().getEnchantmentLevel(
				Enchantment.LUCK);
		...
		for (Rarity rarity : Rarity.values()) {
			...
			if (rarity.equals(0) || rarity.equals(1) || rarity.equals(2)
					|| rarity.equals(3) && checkSkillTier() > 0) {
				...
				if (diceRoll <= dropRate) {
					...
					List<FishingTreasure> fishingTreasures = TreasureConfig
							.getInstance().fishingRewards.get(rarity);

					if (fishingTreasures.isEmpty()) {
						return null;
					}

					treasure = fishingTreasures.get(Misc.getRandom().nextInt(
							fishingTreasures.size()));
					break;
				}
		...
		if (maxDurability > 0) {
			treasureDrop.setDurability((short) (Misc.getRandom()
					.nextInt(maxDurability)));
		}

		if (treasureDrop.getAmount() > 1) {
			treasureDrop.setAmount(Misc.getRandom().nextInt(
					treasureDrop.getAmount()) + 1);
		}
	...
	private void handleTraps() {
		...
		if (Misc.getRandom().nextBoolean()) {
			...
			player.sendMessage(LocaleLoader
					.getString("Fishing.Ability.TH.Poison"));

			ThrownPotion thrownPotion = player.getWorld().spawn(
					fishingCatch.getLocation(), ThrownPotion.class);
			thrownPotion.setItem(new Potion(PotionType.POISON).splash()
					.toItemStack(1));
	...
	private Map<Enchantment, Integer> handleMagicHunter(ItemStack treasureDrop) {
		...
		for (Rarity rarity : Rarity.values()) {
			...
			double dropRate = TreasureConfig.getInstance()
					.getEnchantmentDropRate(getLootTier(), rarity);

			if (diceRoll <= dropRate) {
				fishingEnchantments = TreasureConfig.getInstance().fishingEnchantments
						.get(rarity);
				break;
			}
		...
		for (EnchantmentTreasure enchantmentTreasure : fishingEnchantments) {
			if (validEnchantments
					.contains(enchantmentTreasure.getEnchantment())) {
		...
		// This make sure that the order isn't always the same, for example
		// previously Unbreaking had a lot more chance to be used than any other
		// enchant
		...
		for (EnchantmentTreasure enchantmentTreasure : possibleEnchants) {
			Enchantment possibleEnchantment = enchantmentTreasure
					.getEnchantment();
